...
import { 
  ...
  Bell, Tag, X, Info, Coins, Ticket, Snowflake, Zap,
  CheckCircle
} from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const getStatusInfo = (status: string) => {
    switch (status?.toUpperCase()) {
      ...
      case 'BATAL': return { label: 'Batal', color: 'text-rose-700', bg: 'bg-rose-100', icon: <Trash2 size={12}/> };
      default: return { label: status, color: 'text-gray-700', bg: 'bg-gray-100', icon: <Clock size={12}/> };
    }
  };
// 1. TARUH DI SINI (Di dalam komponen, sebelum return)
  const getStatusConfig = (statusName: string) => {
    const config: any = {
      'MENUNGGU': { label: 'Menunggu', color: 'text-yellow-600', bg: 'bg-yellow-100', icon: <Clock size={18} /> },
      'DIPROSES': { label: 'Diproses', color: 'text-blue-600', bg: 'bg-blue-100', icon: <Package size={18} /> },
      'SELESAI': { label: 'Selesai', color: 'text-green-600', bg: 'bg-green-100', icon: <CheckCircle size={18} /> },
      'BATAL': { label: 'Batal', color: 'text-red-600', bg: 'bg-red-100', icon: <X size={18} /> },
    };
    return config[statusName] || config['MENUNGGU'];
  };
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      // Real-time User Data (Poin, Nama, Alamat)
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-white">
      ...
      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Sinkronisasi Data...</p>
    </div>
  );

  function deleteAddress(id: string): void {
    throw new Error('Function not implemented.');
  }

  const addAddress = async (e: React.MouseEvent<HTMLButtonElement>) => {
  e.preventDefault();
    throw new Error('Function not implemented.');
  }
        ...
        {showNotif && (
          <div className="absolute right-6 top-20 w-80 bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-5 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Aktivitas Pesanan</span>
              <button onClick={() => setShowNotif(false)}><X size={16} className="text-slate-300"/></button>
            </div>
            <div className="max-h-80 overflow-y-auto p-3 space-y-2">
              {activeOrdersList.length > 0 ? activeOrdersList.map(order => (
                <div key={order.id} onClick={() => router.push(`/transaksi/${order.id}`)} className="p-4 hover:bg-slate-50 rounded-2xl flex gap-3 items-center transition-all cursor-pointer border border-transparent hover:border-slate-100">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 relative overflow-hidden group">
              <div className="absolute -right-4 -top-4 text-slate-50 rotate-12 transition-transform group-hover:scale-110">
                <User size={120} />
              </div>
                  ...
                  <div className="flex-1">
                    {isEditingName ? (
                      <div className="flex gap-2">
                        ...
                        <button onClick={handleSaveName} className="text-green-600"><Save size={20}/></button>
                      </div>
                    ) : (
              ...
              <div className="mt-6 space-y-3 pt-6 border-t border-dashed border-slate-100">
                {(userRole === 'admin' || userRole === 'cashier') && (
                  <Link href="/cashier" className="w-full flex items-center justify-center gap-3 py-4 bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase rounded-2xl hover:bg-emerald-100 transition-all">
                    <ClipboardList size={18} /> Dashboard Kasir
                  </Link>
                )}
            ...
            <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2">
                <MapPin size={14} className="text-green-600" /> Alamat Saya
              </h3>
              <div className="space-y-4 mb-8">
                {addresses.map(addr => (
                  <div key={addr.id} className="p-5 bg-slate-50 rounded-3xl relative border border-slate-50 group">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 mt-1 leading-relaxed uppercase">{addr.address}</p>
              ...
              <div className="bg-slate-50 p-6 rounded-[2rem] border-2 border-dashed border-slate-200 space-y-3">
                ...
                <input value={newReceiverPhone} onChange={e => setNewReceiverPhone(e.target.value)} placeholder="NO. WA..." className="w-full bg-white rounded-xl px-4 py-3 text-[10px] font-black uppercase outline-none border border-slate-100 focus:border-green-500" />
                <textarea value={newAddress} onChange={e => setNewAddress(e.target.value)} placeholder="ALAMAT LENGKAP..." className="w-full bg-white rounded-xl px-4 py-3 text-[10px] font-black uppercase outline-none border border-slate-100 h-20" />
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
              <div className="flex items-center justify-between mb-8">
                <div>
                   <h3 className="text-[11px] font-black text-slate-900 uppercase tracking-widest italic">Lacak Belanja</h3>
                   <p className="text-[9px] font-bold text-slate-400 uppercase mt-1">Total {orders.length} Transaksi</p>
                </div>
                {activeOrdersCount > 0 && (
                   <div className="bg-green-100 text-green-600 px-4 py-2 rounded-full flex items-center gap-2 animate-pulse">
                ...
                <div className="space-y-4">
                  ...
                  {orders.length > ordersPerPage && (
                    <div className="flex items-center justify-center gap-2 mt-10">
                      <button onClick={() => setCurrentPage(p => Math.max(1, p-1))} disabled={currentPage === 1} className="p-2 text-slate-300 hover:text-green-600 disabled:opacity-20"><ChevronLeft/></button>
                      {[...Array(Math.ceil(orders.length/ordersPerPage))].map((_, i) => (
                        <button key={i} onClick={() => setCurrentPage(i+1)} className={`w-8 h-8 rounded-xl text-[10px] font-black transition-all ${currentPage === i+1 ? 'bg-green-600 text-white' : 'bg-slate-100 text-slate-400'}`}>{i+1}</button>
                      ))}
                      <button onClick={() => setCurrentPage(p => Math.min(Math.ceil(orders.length/ordersPerPage), p+1))} disabled={currentPage === Math.ceil(orders.length/ordersPerPage)} className="p-2 text-slate-300 hover:text-green-600 disabled:opacity-20"><ChevronRight/></button>
                    </div>
                  )}
                </div>
              )}
