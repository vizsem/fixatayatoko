...
function RestockModal({ product, isOpen, onClose }: any) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      onClose();
    } catch (err) { toast.error("Gagal update data"); }
    setLoading(false);
  };
const handleSync = () => {
  const t = toast.loading("Mensinkronkan data...");
  
  // Membersihkan semua state filter dan pilihan
  setSearchTerm('');
  setCurrentPage(1);
  setSelectedIds([]);
  setShowInactive(false); // Kembalikan ke tampilan produk aktif

  // Memberikan efek delay sedikit agar user merasa ada proses "sinkronisasi"
  setTimeout(() => {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden">
        <div className="bg-blue-600 p-6 text-white flex justify-between items-center">
          <h2 className="font-black uppercase italic tracking-tighter text-lg">Kalkulator Modal Avg</h2>
        ...
        <div className="p-8 space-y-4">
          ...
          <div className="p-5 bg-blue-50 rounded-2xl border border-blue-100">
            <p className="text-[9px] font-black text-blue-600 uppercase mb-1">Estimasi Modal Baru</p>
            <p className="text-xl font-black text-gray-900">Rp {simulasiHargaAvg.toLocaleString()}</p>
          </div>
          <button onClick={handleUpdate} disabled={loading} className="w-full bg-blue-600 text-white p-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-blue-700 transition-all">
...
const handleBulkStatus = async (newStatus: number) => {
  ...
  } catch (err) { toast.error("Gagal memperbarui", { id: t }); }
};
  // States
  ...
  const [selectedProductRestock, setSelectedProductRestock] = useState<any>(null);
  const [showInactive, setShowInactive] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 20;

  const [formData, setFormData] = useState<any>({
    ID: '', Barcode: '', Nama: '', Kategori: 'Umum', Satuan: 'Pcs', 
    Stok: 0, Min_Stok: 5, Modal: 0, Ecer: 0, Harga_Grosir: 0, Min_Grosir: 0, 
    Lokasi: '', Deskripsi: '', URL_Produk: '', Status: 1, warehouseId: '',
    tgl_masuk: new Date().toISOString().split('T')[0], expired_date: ''
  });
  ...
  useEffect(() => {
    ...
    const unsubW = onSnapshot(collection(db, 'warehouses'), (s) => setWarehouses(s.docs.map(d => ({id: d.id, ...d.data()}))));
    const unsubP = onSnapshot(query(collection(db, 'products'), orderBy('updatedAt', 'desc')), (s) => setProducts(s.docs.map(d => ({id: d.id, ...d.data()}))));
  ...
  const handleExport = () => {
    ...
    } catch (err) { toast.error("Gagal export!", { id: t }); }
  };
  ...
  const handleFileUpload = (e: any) => {
    ...
    reader.onload = async (evt) => {
      ...
      try {
        ...
        await batch.commit();
        toast.success("Berhasil!", { id: t });
      } catch (err) { toast.error("Gagal!", { id: t }); }
    };
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen text-black">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-black italic uppercase tracking-tighter">ATAYAMARKET</h1>
          <p className="text-[10px] font-bold text-gray-400 uppercase">Product Management</p>
        </div>
  ...
  >
    <RefreshCw size={16} />
          ...
          <button onClick={downloadTemplate} className="bg-white border px-4 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2"><FileSpreadsheet size={14}/> Template</button>
          <button onClick={() => fileInputRef.current?.click()} className="bg-orange-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 shadow-md"><Upload size={14}/> Import</button>
          <button onClick={handleExport} className="bg-emerald-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 shadow-md"><Download size={14}/> Export</button>
          <button onClick={() => setShowAddModal(true)} className="bg-black text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 shadow-xl"><Plus size={14}/> Item Baru</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-white p-5 rounded-[2rem] border shadow-sm flex items-center gap-4">
          ...
          <div><p className="text-[10px] font-black text-gray-400 uppercase">Produk</p><p className="text-xl font-black">{stats.totalJenis}</p></div>
        </div>
        <div className="bg-white p-5 rounded-[2rem] border shadow-sm flex items-center gap-4">
           ...
           <div className="flex gap-2">
             ...
             {selectedIds.length > 0 && (
               <button 
                ...
                className="bg-red-600 text-white px-4 py-3 rounded-2xl text-[10px] font-black uppercase flex items-center gap-2 animate-in fade-in zoom-in"
               >
             ...
             <button 
              ...
              className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase border transition-all flex items-center gap-2 ${showInactive ? 'bg-red-600 text-white border-red-600' : 'bg-white text-gray-400'}`}
             >
  ...
  {(() => {
    ...
    if (isValid) {
      return (
        <img 
          ...
          onError={(e) => {
            // Jika error, ganti ke icon cadangan agar tidak kosong
            const target = e.currentTarget;
            target.classList.add('hidden'); 
            if (target.parentElement) {
              const iconPlaceholder = document.createElement('div');
              iconPlaceholder.className = "text-gray-300";
              iconPlaceholder.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.5 4h-5L7 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3l-2.5-3z"/><circle cx="12" cy="13" r="3"/></svg>';
              target.parentElement.appendChild(iconPlaceholder);
            }
          }}
        />
      );
    ...
    return <Camera size={16} className="text-gray-300" />;
                        ...
                        <div>
                          <p className="text-[8px] font-black text-blue-600 uppercase tracking-tighter">ID: {p.ID}</p>
                          <h3 className="font-black text-gray-900 uppercase text-xs leading-none mb-1">{p.Nama}</h3>
                          <p className="text-[8px] text-gray-400 font-bold uppercase">{p.Kategori}</p>
                        </div>
                      </div>
                    </td>
                    <td className="p-6">
                      <p className={`font-black text-xs ${p.Stok <= p.Min_Stok ? 'text-red-600' : 'text-gray-900'}`}>{p.Stok} {p.Satuan}</p>
                      <p className="text-[8px] font-black text-gray-400 uppercase flex items-center gap-1"><Warehouse size={10}/> {whName}</p>
                    </td>
                    <td className="p-6">
                      <p className="text-[8px] font-black text-blue-600 uppercase">Modal: Rp{(p.hargaBeli||p.Modal||0).toLocaleString()}</p>
                      <p className="font-black text-xs text-emerald-600">Jual: Rp{(p.Ecer||0).toLocaleString()}</p>
                    </td>
                    <td className="p-6">
                      <p className="text-[8px] font-black text-gray-400 uppercase">IN: {p.tgl_masuk || '-'}</p>
                      <p className={`text-[8px] font-black uppercase flex items-center gap-1 ${isExpired ? 'text-red-600 animate-pulse' : 'text-orange-500'}`}>
        ...
        <div className="p-6 bg-gray-50/50 flex justify-between items-center border-t border-gray-100">
          <div className="text-[10px] font-black uppercase text-gray-400">Hal {currentPage} / {totalPages || 1} — {filteredAndSorted.length} Produk</div>
          <div className="flex gap-2">
      ...
      {/* Modal Add Item Baru & Restock - (Gunakan kode modal Anda yang sudah ada di sini) */}
...
function setSelectedIds(arg0: never[]) {
  throw new Error('Function not implemented.');
}

function setShowInactive(arg0: boolean) {
  throw new Error('Function not implemented.');
}
