...
function BulkPrintContent() {
  ...
  if (loading) return <div className="p-10 text-center font-black uppercase text-slate-400 animate-pulse tracking-widest">Generating Labels...</div>;
  if (products.length === 0) return <div className="p-10 text-center font-black text-red-500 uppercase">Data Kosong / Tidak ada produk dipilih</div>;
    ...
    <div className="bg-gray-100 min-h-screen text-black font-sans pb-20">
      <div className="max-w-4xl mx-auto p-4 no-print pt-8">
        ...
        <div className="bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-2xl text-xs font-bold flex items-start gap-3 mb-8">
          ...
          <p>Pastikan ukuran kertas di pengaturan printer Anda sudah disesuaikan (contoh: Lebar 50mm x Tinggi 30mm), hilangkan Margin, dan atur skala ke 100% sebelum mencetak.</p>
        </div>
        
        <h2 className="text-sm font-black uppercase tracking-widest text-gray-400 mb-4 ml-2">Preview Stiker:</h2>
      </div>

      {/* --- AREA CETAK --- */}
      <div className="print-only-container flex-col gap-0">
        {products.map((product, index) => {
          ...
          return (
            <div 
              ...
              className="bg-white border border-gray-300 overflow-hidden print-exact-size page-break-here"
              style={{ 
                width: '50mm', 
                height: '30mm',
                padding: '2mm',
      ...
      <style dangerouslySetInnerHTML={{
        __html: `
          @media print {
            ...
            /* Menghilangkan page break di elemen terakhir agar tidak ada halaman kosong di akhir */
            .page-break-here:last-child {
...
export default function BulkPrintPage() {
  return (
    <Suspense fallback={<div className="p-10 text-center font-black uppercase">Loading...</div>}>
      <BulkPrintContent />
    </Suspense>
  );
}
