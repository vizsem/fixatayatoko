...
import { 
  ...
  CheckCircle2, Gift, Zap, Snowflake 
} from 'lucide-react';
...
export default function VoucherExchangePage() {
    ...
    try {
      ...
      toast.success((t) => (
        <span className="flex flex-col gap-1">
          <b className="text-xs uppercase">Berhasil Ditukar!</b>
          ...
          <button 
            ...
            className="mt-2 bg-black text-white text-[8px] font-black py-1 px-2 rounded-lg uppercase"
          >
            Salin Ulang
          </button>
        </span>
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center italic font-black uppercase tracking-widest">Loading...</div>;
    ...
    <div className="min-h-screen bg-gray-50 text-slate-900 pb-10">
      ...
      <div className="bg-white p-6 shadow-sm border-b sticky top-0 z-10">
        <div className="max-w-xl mx-auto flex items-center justify-between">
          ...
          <div className="text-center">
            <h1 className="font-black italic uppercase text-lg tracking-tighter">Voucher Center</h1>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Tukar Poin Jadi Keuntungan</p>
          </div>
      ...
      <div className="max-w-xl mx-auto p-4 space-y-6">
        ...
        <div className="bg-black rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60 mb-2">Saldo Poin Anda</p>
          ...
          {userData?.isPointsFrozen && (
            <div className="mt-4 flex items-center gap-2 bg-red-600/20 text-red-400 p-3 rounded-2xl border border-red-600/30">
              <Snowflake size={16} />
              <span className="text-[10px] font-black uppercase">Akun dibekukan sementara</span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          <h3 className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-2">Voucher Tersedia</h3>
          
          {VOUCHER_LIST.map((v) => {
            const isAffordable = (userData?.points || 0) >= v.cost;
            return (
              <div key={v.id} className="bg-white rounded-[2rem] p-5 shadow-sm border border-gray-100 flex items-center justify-between group">
                <div className="flex items-center gap-4">
                  <div className={`w-14 h-14 rounded-2xl ${v.color} flex items-center justify-center text-white shadow-lg shadow-gray-200`}>
                    <Ticket size={24} />
                  </div>
                  <div>
                    <h4 className="text-sm font-black uppercase italic leading-tight">{v.name}</h4>
                    <p className="text-[10px] font-bold text-gray-400 flex items-center gap-1 mt-1 uppercase">
        ...
        <div className="bg-blue-50 p-6 rounded-[2rem] border border-blue-100">
          <h5 className="text-[10px] font-black uppercase text-blue-600 mb-2 flex items-center gap-2">
            <Gift size={14} /> Cara Menggunakan Voucher
          </h5>
          <p className="text-[10px] text-blue-800/70 font-bold leading-relaxed uppercase">
